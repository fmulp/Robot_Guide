...
class RobotGuideApp(App):
    ...
    def show(self, *widgets):
        global gRoot        
        gRoot = self.root
        for wdg in self.root.children[:]:
            self.root.remove_widget(wdg)
        for widget in widgets:
            self.root.add_widget(widget)
        if not widgets:
            self.root.add_widget(self.root.ids.MainButton)
            self.root.add_widget(self.root.ids.Logo, index=1)
    ...
    def move_to_exhibit(self, dt=0, index=None):
        ...
        self.show(
                Label(text=data['description'], font_size=self.root.ids.MainButton.font_size), 
                Image(source=data['visualurl'])
            )
